...
<!-- INI KONTEN -->
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        <li><a href="{{ url('gabsi') }}" class="active"><i class="fa fa-dashboard"></i> Daftar Organisasi</a></li>
      </ol>
    </section>
        ...
        <div class="col-xs-12">
          <div class="box">
            <div class="box-header">
              <h3 class="box-title">Daftar Organisasi</h3><br>
              <a class="btn btn-primary" href="{!! action('GabsiController@create') !!}">TAMBAH</a>
              @if(session('status'))
            ...
            <div class="box-body">
              <table id="example1" class="table table-bordered table-striped">
                ...
                <tbody>
                @foreach ($gabsis as $key => $gabsi)
                <tr>
                  <td class="text-center">{{ $key+1}}</td>
                  <td class="text-center">{{ $gabsi->nama }}</td>
                  <td class="text-center">{{ $gabsi->alamat }}</td>
                  <td class="text-center">{{ $gabsi->no_tlp }}</td>
                  <td class="text-center">{{ $gabsi->email }}</td>
                  <td class="text-center">{{ $gabsi->keterangan }}</td>
                  <td class="text-center"><img src="{{ $gabsi->foto }}" style="width:100px" class="img-thumbnail"></td>
                  <td>
                    <a class="btn btn-sm btn-primary" href="{!! action('GabsiController@edit', $gabsi->id) !!}">Edit</a>
                  </td>
                  <td>
                    <form action="{{ route('gabsi.destroy', $gabsi->id) }}" method="POST"> 
                    {{ method_field("DELETE") }} 
                    {{ csrf_field() }}
