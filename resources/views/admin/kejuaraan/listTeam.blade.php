...
<!-- INI KONTEN -->
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        <li><a href="{{ url('kejuaraan') }}" class="active"><i class="fa fa-dashboard"></i> Daftar Peserta</a></li>
      </ol>
    </section>
    ...
    <section class="content">
      <div class="row">
        <div class="col-md-12">
          <!-- INI KALENDER -->
          <a class="btn btn-success btn-sm update-cart" href="{{url("kejuaraan/absensi/absensiall/".$pendaftar->id)}}">Absen Semua</a>
        ...
        <div class="col-md-12">
          ...
          <div class="box">
            <div class="box-header">
              <h3 class="box-title">{{ $pendaftar->namaTeam }}</h3>
              @if($pendaftar->pc != "")
                <h4>Official PC : {{ $pendaftar->pc }}</h4>
              @elseif($pendaftar->npc != "")
                <h4>Official NPC : {{ $pendaftar->npc }}</h4>
              @else
                <h4></h4>
              @endif
              @if(session('status'))
              ...
              @endif
              <a href='{{ url('kejuaraan/absensi/'.$idkejuaraan) }}' class="btn btn-success btn-sm update-cart">Go Back</a>
            </div>
            <!-- /.box-header -->
            ...
            <div class="box-body">
              <table id="example1" class="table table-bordered table-striped">
                ...
                <tr>
                  ...
                  <td>{{ $p->gabsi }}</td>
                  @if($p->statusAbsen == 0)
                  <td class="actions" id="td_{{ $p->id }}" data-th="">
                        <a class="btn btn-success btn-sm update-cart" data-id="{{ $p->id }}" 
                            onclick="if(confirm('apakah anda yakin peserta tersebut sudah datang?')){absen({{$p->id}})}">Absen</a>
                    </td>
                    @else
                    <td>
                      Sudah Absen
...
<script type="text/javascript">
    function absen(id){
        $.ajax({
          type:'POST',
          url:'{{url("kejuaraan/absensi/update")}}',
